...
<template>
  <UMain class="mt-20 px-2">
    <UContainer class="relative min-h-screen">
      <UPage v-if="page">
        <ULink to="/blog" class="text-md flex items-center gap-1">
          <UIcon name="lucide:chevron-left" />
          Blog
        </ULink>
        <div class="flex flex-col gap-3 mt-8">
          <div
            class="flex text-md text-muted items-center justify-center gap-2"
          >
            <span v-if="page.date">
        ...
        <UPageBody class="max-w-3xl mx-auto">
          ...
          <div class="flex items-center justify-end gap-2 text-md text-muted">
            <UButton
              size="sm"
              variant="link"
              color="neutral"
